...
unfold optimise_instr. simpl in *. unfold sound_env in senv. simpl in *. unfold prep_selected_block in *; simpl in *. destruct ( find_function m fn) eqn:?. destruct (find_block (blks (df_instrs d)) bk) eqn:?. assert ( Some (prep_block b) = Some (prep_block b)) by auto. apply senv in H. eapply  val_correct in H; eauto. rewrite <- H. try_resolve. try_resolve. try_resolve. try_resolve. try_resolve. try_resolve. 
