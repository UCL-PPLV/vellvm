...
Fixpoint change_env (i:local_id) (s:value) (a:aenv) :=
  match a with
  | nil => cons (i, s) nil
  | hd :: tl => if (decide ((fst hd) = i)) then ((fst hd), s) :: tl else change_env i s tl
  end.


Lemma lookup_first : forall id dv e, lookup_env ((id, dv) :: e) id = Some dv.
Proof. intros. unfold lookup_env. simpl in *. destruct (AstLib.RawID.eq_dec id id). eauto. contradiction n. eauto. Qed.

Hint Resolve lookup_first.
...
Print aenv. Print areg.
Print eval_op. Print err. Print instr_id. Print SV. Print Expr.
Definition transfer (a:aenv) (i:instr_id * cmd) : aenv :=
  ...
  | (IId loc), CFG.Step ins => match ins with
                    | INSTR_Op val => match val with
                                      | SV (VALUE_Integer i) => (loc, (DV (VALUE_Integer i))) :: a
                                                                         | _ => a
                                      end
                    | _ => a
...
Proof. (* intros. unfold find_tuple in *. induction l. simpl in *. inversion H0. simpl in *. destruct a. simpl in *.
       ...
       exists ( (l0, v) :: x). exists x0. auto. *) Admitted.
...
inversion H; subst. apply IHl in H4; eauto. inversion H4; subst. inversion H1. subst. inversion H2. subst. exists ((l0, v) :: x). simpl in *. exists x0. exists x1. eauto.*) Admitted.
...
Print lookup_env.
Lemma In_to_look : forall a b s,  lookup_env s a = Some b -> In (a, b) s.
Proof. intros. induction s. simpl in *. unfold lookup_env in *. simpl in *. inversion H.
destruct a0. simpl in *.   unfold lookup_env in *. simpl in *. simpl in *. destruct ( AstLib.RawID.eq_dec a l). subst. inversion H. subst. left. auto. simpl in *. apply IHs in H. right. auto. Qed.


Definition Subset' s s' := forall a b, lookup_env s a = Some b -> lookup_env s' a = Some b.
...
Print Subset. Print areg.

Lemma convert_in : forall s a b, lookup_env s a = Some b -> In (a, b) s.
  Proof. intros. induction s. simpl in *. inversion H. simpl in *.  unfold lookup_env in *. simpl in *. destruct a0.  simpl in *. destruct ( AstLib.RawID.eq_dec a l). simpl in *. inversion H. subst. inversion H. auto. auto. Qed.

Print value_analysis.


  Definition Subset1 (s:env) s' := forall a b,  In (a, b) s -> In (a, b) s'.

Lemma convert_in1 : forall s s' a b, Subset1 s s' -> lookup_env s a = Some b -> lookup_env s' a = Some b.
Proof. intros.

       generalize H0. intros. apply convert_in in H0. generalize a, b. induction s'. simpl in *. clear H1. intros. admit. simpl in *. intros. destruct a0. simpl in *. unfold lookup_env. simpl in *. destruct ( AstLib.RawID.eq_dec a1 l). subst. unfold Subset1 in *.
...
Definition sound_env (m:mcfg) (p:pc) (e:env) :=
  forall prep, prep_selected_block m p = Some prep ->
               Subset1 (value_analysis prep (pt p)) e.
...
destruct (decide (fst blk_term = fst blk_term)). intros. 


simpl in *. inversion H0. contradiction n; eauto. simpl in *. intros. simpl in *. destruct p0. simpl in *. simpl in *. admit. inversion H7.*) Admitted.
 ...
 subst.           remember ( find_function m fn0).  destruct o; simpl in *; inversion H1.
           remember (find_block (blks (df_instrs d)) bk0). destruct o; simpl in *; inversion H1.
           remember (block_to_cmd b pt0). destruct o; simpl in *.



           destruct p; simpl in *. destruct c. destruct o; inversion H1; subst. destruct pt0, i; inversion H1; subst; simpl in *.
           +remember (eval_op e None op); inversion H1. destruct e1; simpl in *; inversion H1; subst. unfold sound_env in *. simpl in *. eapply test5 in H; eauto.  unfold prep_selected_block in *; simpl in *.
            rewrite <- Heqo. rewrite <- Heqo0. rewrite <- Heqo in senv. rewrite <- Heqo0 in senv. intros. inversion H2. subst. apply senv in H2. rewrite H. clear H. unfold transfer. simpl in *.
            destruct op. simpl in *. admit.
           +(*6*) destruct fn1. 
...
Lemma step_preserves_sound_state : forall m st st1,   (forall b, NoDup ((map fst (blk_code b)) ++ ( blk_term_id b ) :: nil))  -> sound_state m st  -> stepD m st = Step st1 -> sound_state m st1.
Proof. intros. constructor. eapply  step_preserves_sound_stack  in H; eauto. eapply  step_preserves_sound_env in H; eauto. Qed.
...
remember (find_function m fn0). destruct o0. simpl in *. remember (find_block (blks (df_instrs d0)) bk0). destruct o0. simpl in *. intros. inversion H2. subst. apply sstack_env in H2. unfold Subset in *. simpl in *. intros. right. apply H2. auto. intros. inversion H2. intros. inversion H2. inversion H1.

destruct s. inversion H1. destruct f. simpl in *. inversion H1. inversion H1. subst. inversion H0. subst. simpl in *. inversion sstack. subst. simpl in *. unfold sound_env in *. simpl in *. unfold prep_selected_block in *. simpl in *. remember ( find_function m fn0). destruct o0. simpl in *.
...
simpl in *. unfold sound_env in *. simpl in *. unfold prep_selected_block in *. simpl in *. rewrite <- Heqo. generalize Heqo0. intros. eapply find_block_equiv in Heqo0. subst. rewrite <- Heqo2 in H1. inversion H1. subst. unfold Subset. simpl in *. intros. destruct b0. simpl in *. unfold value_analysis in *. simpl in *. unfold prep_block in *. simpl in *. unfold blk_term_id in *. simpl in *. destruct blk_code in *. simpl in *. simpl in *. unfold is_left in *. simpl in *. destruct ( decide (fst blk_term = fst blk_term)). simpl in *. inversion H2. contradiction n; eauto. simpl in *. destruct p0. simpl in *. unfold is_left in *. simpl in *. destruct ( decide (i = i)). simpl in *. inversion H2. contradiction n. eauto. inversion H5. Qed.
...
Lemma jump_preserves_sound_state : forall m st st1,   (forall b, NoDup ((map fst (blk_code b)) ++ ( blk_term_id b ) :: nil))  -> sound_state m st  -> stepD m st = Jump st1 -> sound_state m st1.
Proof. intros. constructor. eapply  jump_preserves_sound_stack  in H; eauto. eapply  jump_preserves_sound_env in H; eauto. Qed.*) Admitted.*)