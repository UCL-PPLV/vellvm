...
Definition eval_exp (i:exp) (s:env) : option exp :=
  match i with
...
Print aenv.
Definition transfer (acc:aenv) (a:areg) := a :: acc.


Fixpoint calculate_previous1 (acc l:aenv) (i:nat)  : aenv :=
  match l with
  | nil => nil
  | cons hd tl => if beq_nat (fst hd) i then nil else cons hd (calculate_previous1 (transfer acc hd) tl i)
  end.


Definition calculate_previous (l:aenv) (i:nat) := calculate_previous1 nil l i.

Print list_to_tuple.
Lemma test123 : forall l i t i1,  list_to_tuple l i = Some (t, Some i1) -> calculate_previous l i1 = transfer (calculate_previous l i) (i, t).
  Proof. intros. unfold calculate_previous. simpl in *. induction l. simpl in *. inversion H. simpl in *. destruct a. simpl in *. destruct (PeanoNat.Nat.eqb i0 i). simpl in *. destruct (PeanoNat.Nat.eqb i0 i1). simpl in *.
