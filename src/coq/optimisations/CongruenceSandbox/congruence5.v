...
Lemma  compare_seq_implies_starting_state : forall p p1 blk_code  mem s0 s1, compare_seq (exec_code1 p blk_code (visitem mem s0))
                                                                                                  (exec_code1 p1 blk_code (tauitem mem s1)) -> False.
Proof. intros. destruct blk_code; simpl in *; eauto; destruct s0; eauto; inversion H; subst; try inversion H3. inversion H4.
       subst.  destruct p0. simpl in *. destruct s1. clear H. simpl in *. destruct p0. simpl in *. destruct p0. simpl in *.
       destruct ( find_function p1 fn); simpl in *; eauto. destruct ( find_block (blks (df_instrs d)) bk); simpl in *; eauto. unfold block_to_cmd in *. simpl in *. destruct b. unfold blk_term_id in *. simpl in *. destruct blk_term. simpl in *. destruct ( decide (i1 = pt)); simpl in *; eauto. inversion H2; subst; simpl in *. destruct ( find_instr blk_code0 pt i1); simpl in *; eauto. destruct p0; simpl in *; eauto. destruct o; simpl in *; eauto. destruct pt, i0; simpl in *; eauto. destruct ( eval_op e None op); simpl in *; eauto. inversion H2; subst. inversion H2. destruct fn0; simpl in *; eauto.  destruct i0; simpl in *; eauto. destruct  (find_function_entry p1 id0); simpl in *; eauto. destruct f; simpl in *; eauto. destruct (         map_monad
            (fun '(t, op) => eval_op e (Some t) op) args); simpl in *; eauto. inversion H2; subst. inversion H2. inversion H2. inversion H2. inversion H2. destruct ptr; simpl in *; eauto. destruct ( eval_op e (Some t1) v0); simpl in *; eauto. inversion H2; simpl in *; eauto. destruct v1; simpl in *; eauto. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. admit. (*15*) inversion H2; simpl in *; eauto. inversion H2. destruct val, ptr; simpl in *; eauto. destruct (eval_op e (Some t0) v0), ( eval_op e (Some t1) v1); simpl in *; eauto. inversion H2. inversion H2. inversion H2. destruct v3; simpl in *; inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. subst. inversion H4. subst. destruct p0. simpl in *. destruct s1. destruct p0. simpl in *. destruct (incr_pc p1 p0); simpl in *. destruct (pt p0); simpl in *; eauto. destruct i0; simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. inversion H2. inversion H2. destruct fn; simpl in *; eauto. destruct i0; simpl in *; eauto. destruct   (find_function_entry p1 id0); simpl in *; eauto. destruct f; simpl in *; eauto. destruct (         map_monad
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          (fun '(t, op) => eval_op e (Some t) op) args); simpl in *; eauto. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. destruct ptr; simpl in *; eauto. destruct ( eval_op e (Some t0) v); simpl in *; eauto. inversion H2. destruct v0; simpl in *; try inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. inversion H2. destruct i0; simpl in *; try inversion H2; subst. destruct fn. destruct i0; simpl in *; inversion H2. destruct   (find_function_entry p1 id); simpl in *; eauto.
       destruct f; simpl in *; eauto. 
destruct (          map_monad
            (fun '(t, op) => eval_op e (Some t) op) args); simpl in *; eauto; inversion H3. destruct t; simpl in *; inversion H5. inversion H3. destruct val, ptr. destruct ( eval_op e (Some t) v), ( eval_op e (Some t0) v0); simpl in *; inversion H1. destruct v2; inversion H1. inversion H2. Admitted.
...
Lemma jump_equiv : forall (r: Trace () -> Trace () -> Prop) o m br1 e s fn bk pt mem, trace_equiv_step (upaco2 (trace_equiv_step (X:=())) r)
    match
      match
        match
          match (do st <- jump m fn bk br1 e s; Jump st) with
          | Step s' =>
              Tau ({| fn := fn; bk := bk; pt := pt |}, e, s) (step_sem m s')
          | Jump s' =>
              Tau ({| fn := fn; bk := bk; pt := pt |}, e, s) (step_sem m s')
    ...
    match
      match
        match
          match (do st <- jump (optimise_modul o m) fn bk br1 e s; Jump st) with
          | Step s' =>
              Tau ({| fn := fn; bk := bk; pt := pt |}, e, s)
                (step_sem (optimise_modul o m) s')
          | Jump s' =>
              Tau ({| fn := fn; bk := bk; pt := pt |}, e, s)
                (step_sem (optimise_modul o m) s')
          | Obs (Fin s0) => Vis (Fin s0)
          | Obs (Err s0) => Vis (Err s0)
          | Obs (Eff m1) =>
              Vis (Eff (effects_map (step_sem (optimise_modul o m)) m1))
          end
        with
        | Vis v0 => Vis (trace_map (fun _ : state => ()) <$> v0)
        | Tau _ k => Tau () (trace_map (fun _ : state => ()) k)
...
Proof. intros m o test. pcofix CIH. intros. pfold.
       ...
       destruct t. destruct v. destruct ( eval_op e (Some t) v); simpl in *; eauto.
       destruct s; simpl in *; eauto. destruct f; simpl in *; eauto. destruct s; simpl in *; eauto.
       destruct f; simpl in *; eauto. destruct v; simpl in *; eauto. destruct ( eval_op e (Some t) v); simpl in *; eauto. destruct v0; simpl in *; eauto. destruct (StepSemantics.Int1.eq x StepSemantics.Int1.one); simpl in *; eauto. eauto. simpl in *; eauto. simpl in *; eauto. simpl in *; eauto. simpl in *; eauto.
       ...
       destruct ( decide (i = pt)); subst; simpl in *.




assert (pt = pt) by auto. apply n in H2. inversion H2.



 destruct a. unfold optimise_instruction in *. simpl in *. destruct ( decide (pt = i0)); simpl in *; eauto. destruct pt, (o i1), i1; simpl in *; eauto.
       ...
       destruct ( eval_op e None op); simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. destruct (eval_op e None op); simpl in *; eauto. destruct (eval_op e None op), fn0; simpl in *; eauto. destruct i1; simpl in *; eauto. destruct  (find_function_entry (optimise_modul o m) id0); simpl in *; eauto. destruct f; simpl in *; eauto. destruct (map_monad (fun '(t1, op0) => eval_op e (Some t1) op0) args); simpl in *; eauto.
...
unfold optimise_code in H1.

       

       destruct blk_code. simpl in *. admit.
       inversion H1; subst. clear IHblk_code; clear H1; clear H5. destruct p. simpl in *.
       destruc
       ...
       admit. destruct i1; simpl in *; eauto. destruct (find_function_entry (optimise_modul o m) id0); simpl in *; eauto. destruct f; simpl in *; eauto.
       destruct ( map_monad (fun '(t1, op0) => eval_op e (Some t1) op0) args); simpl in *; eauto.
