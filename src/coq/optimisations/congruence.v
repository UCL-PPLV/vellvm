...
destruct (decide (i = i1)), (decide (i1 = i1)), (decide (i = pt)), (decide (pt = i1)); simpl in *; subst; simpl in *.
...
destruct (jump prog1 fn bk br1 e s); simpl; eauto.
rewrite H6.
destruct (jump prog1 fn bk br2 e s); simpl; eauto.
rewrite H6.
destruct (jump prog1 fn bk br e s); simpl; eauto.
...
destruct (eval_op e (Some t0) v), (eval_op e (Some t1) v0); simpl; auto.
destruct  v2; simpl; eauto.
...
destruct (StepSemantics.Int1.eq x StepSemantics.Int1.one); simpl; eauto.
(*JUMP*)
unfold jump. unfold find_block_entry.
simpl.


generalize H. intro fulldef.
unfold fulldefinition in fulldef.

specialize (fulldef fn).

remember (find_function prog fn) as jumpfindfunc.
remember (find_function prog1 fn) as jumpfindfunc1.

destruct jumpfindfunc, jumpfindfunc1.
specialize (fulldef d d0). destruct fulldef. clear H5.
destruct H7. clear H5. unfold is_left in *.
destruct (decide (df_args d = df_args d0)). destruct H7.
clear H5. subst. destruct d, d0; simpl in *. specialize (H7 df_instrs1 df_instrs2).
destruct H7. clear H5. destruct H7. clear H5.
destruct (decide (init df_instrs1 = init df_instrs2)); simpl in *.
destruct H7. clear H5. subst. specialize (H7 br1).

remember (find_block (blks df_instrs1) br1) as jumpfindblock.
remember (find_block (blks df_instrs2) br1) as jumpfindblock1.
destruct jumpfindblock, jumpfindblock1. specialize (H7 b b0).

destruct H7. clear H5. destruct H7. clear H5.
unfold block_to_entry. simpl in *.



unfold lift_err_d.

unfold monad_fold_right.
...
destruct H13.
destruct H14.
destruct (decide (blk_phis b = blk_phis b0)).


unfold blk_entry_pc.
unfold blk_entry_id.
unfold blk_term_id.
destruct (decide (blk_term b = blk_term b0)).
rewrite H14; rewrite H15.
unfold hd_fst_block_equal in H11.
unfold hdb in H11. rewrite e0.


 destruct b, b0. simpl in *.
unfold tl_block_equal in H13. destruct blk_code, blk_code1.
simpl in *. inversion H11. inversion H11. destruct p. inversion H11.
destruct p, p0. simpl in *. destruct (decide (blk_id = blk_id1)). 
subst.
remember  ((fix
               ...
               M A :=
                 ...
                 | [::] => mret a
                 | x0 :: xs =>
                     monad_fold_right A B M H5 H7 f xs a
                     ≫= (fun y : A => f y x0)
                 end)) as Q.
rewrite <- HeqQ.
...
destruct H7. inversion H5.
destruct H7. inversion H5.
specialize (fulldef d d). destruct fulldef. destruct H7. inversion H7.
specialize (fulldef d d). destruct fulldef. inversion H5.
inversion Heqfind_func_fn1.
...
destruct H7. clear H5. destruct H7. clear H5.
destruct (decide (init df_instrs1 = init df_instrs2)); simpl in *.
destruct H7. clear H5. subst. specialize (H7 br2).

remember (find_block (blks df_instrs1) br2) as jumpfindblock.
remember (find_block (blks df_instrs2) br2) as jumpfindblock1.
destruct jumpfindblock, jumpfindblock1. specialize (H7 b b0).
...
destruct H7. clear H5. destruct H7. clear H5.
destruct (decide (init df_instrs1 = init df_instrs2)); simpl in *.
destruct H7. clear H5. subst. specialize (H7 br).

remember (find_block (blks df_instrs1) br) as jumpfindblock.
remember (find_block (blks df_instrs2) br) as jumpfindblock1.
destruct jumpfindblock, jumpfindblock1. specialize (H7 b b0).
...
apply n in e3. inversion e3.
assert (pt = pt) by auto.
apply n in H5. inversion H5.
...
destruct (StepSemantics.Int1.eq x StepSemantics.Int1.one); simpl; auto.
...
specialize (H7 {|
       ...
       blk_phis := blk_phis0;
       blk_code := (i1, i2) :: blk_code0;
       blk_term := (pt, TERM_Br (t, v) br1 br2) |}{|
       blk_id := blk_id0;
       blk_phis := blk_phis0;
       blk_code := (i1, i2) :: blk_code0;
       blk_term := (pt, TERM_Br (t, v) br1 br2) |}). destruct H7. inversion H5.
destruct H7. inversion H5.
destruct H7. inversion H5.
specialize (fulldef d d). destruct fulldef. destruct H7. inversion H7.
specialize (fulldef d d). destruct fulldef. inversion H5.
...
specialize (H7 {|
       ...
       blk_phis := blk_phis0;
       blk_code := (i1, i2) :: blk_code0;
       blk_term := (pt, TERM_Br_1 br) |}{|
       blk_id := blk_id0;
       blk_phis := blk_phis0;
       blk_code := (i1, i2) :: blk_code0;
       blk_term := (pt, TERM_Br_1 br) |}). destruct H7. inversion H5.
destruct H7. inversion H5.
destruct H7. inversion H5.
specialize (fulldef d d). destruct fulldef. destruct H7. inversion H7.
specialize (fulldef d d). destruct fulldef. inversion H5.
...
-specialize (H7 b b). destruct H7. destruct H7. inversion H7. 
-specialize (H7 b b). destruct H7. inversion H5.
-specialize (H7 {| blk_id := blk_id0; blk_phis := blk_phis0;  blk_code := (i1, i0) :: blk_code0; blk_term := (i, t) |} {| blk_id := blk_id0; blk_phis := blk_phis0; blk_code := (i1, i0) :: blk_code0; blk_term := (i, t) |}).
destruct H7. inversion H5.
-destruct H7; inversion H5.
-destruct H7; inversion H5.

-specialize (funcentry d d). destruct funcentry. destruct H7. inversion H7. (*here*)
-specialize (funcentry d d). destruct funcentry. inversion H5. 
-specialize (funcentry {|df_prototype := df_prototype; df_args := df_args0; df_instrs := df_instrs |}
{|df_prototype := df_prototype; df_args := df_args0; df_instrs := df_instrs |}).
destruct funcentry. inversion H5.
...
destruct t0; simpl in *; eauto.
destruct v; simpl in *; eauto.
destruct (eval_op e (Some t0) v); simpl in *; eauto.
