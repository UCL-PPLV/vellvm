...
Definition exec_code1 (mem:memory) (m:mcfg) (s:state) :=
  match (stepD m s) with
...
| eff_eq : forall mem mem1 s s1, eq_result (visitem mem1 (Eff s)) (visitem mem (Eff s1))
.




Definition correct_instr (P:mcfg -> state -> Prop) (o:opt) (m:mcfg) :=
  forall mem s (sstate: P m s),
    eq_result (exec_code1 mem m s)
              (exec_code1 mem (modul_opt o m) s).
