...
Lemma id_trace_eq : forall {X} (d:Trace X),
  d = id_match_trace d.
Proof.
  destruct d; auto.
Qed.
...
CoFixpoint trace_map {A B} (f:A -> B) (d:Trace A) : Trace B :=
  match d with
  | Vis v => Vis (fmap (trace_map f) v)
  | Tau s k => Tau (f s) (trace_map f k)
  end.

Instance trace_functor : Functor Trace := fun A B => @trace_map A B.
