...
Definition optimisation := definition (list cfg) -> definition (list cfg).


Definition optimise (m:mcfg) (o:optimisation) : mcfg :=
 {|
  m_name := (m_name m);
  m_target := (m_target m);
...
Definition correct (P : modul (list block) -> Prop) (o:optimization) :=
  forall (m:modul (list block)) m_semantic m_opt_semantic,
    ...
    mcfg_of_modul (optimize m o) = Some m_opt_semantic ->forall s a b, (Reason_sem m_semantic s) = a -> (Reason_sem m_opt_semantic s) = b ->
 trace_equiv a b.
...
Theorem trace_equiv_step_sem: forall a s, trace_equiv (Reason_sem a s) (Reason_sem a s).
Proof. intros. remember (Reason_sem a s) as b. auto. Qed.

Hint Resolve trace_equiv_step_sem.

Theorem trace_equiv_step_sem_equiv : forall s a b, a = b -> trace_equiv (Reason_sem a s) (Reason_sem b s).
