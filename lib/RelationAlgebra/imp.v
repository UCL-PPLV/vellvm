...
Inductive prog :=
| skip
| ass (x: loc) (e: state -> nat)
| seq (p q: prog)
| if_ins (t: dset state) (p q: prog)
| while_ins (t: dset state) (p: prog).

(** notations *)
Bind Scope imp_scope with prog.
Delimit Scope imp_scope with imp.
Notation "x  <- y" := (ass x y) (at level 90): imp_scope.
Notation "p  ;; q" := (seq p%imp q%imp) (left associativity, at level 101): imp_scope.
Arguments if_ins _%ra _%imp _%imp.
Arguments while_ins _%ra _%imp.
...
Fixpoint bstep (p: prog): rel state state :=
  match p with
    ...
    | if_ins b p q => [b] * bstep p + [!b] * bstep q
    | while_ins b p => ([b] * bstep p)^*  *  [!b]
  end.

(** ** using an inductive predicate, as in standard textbooks *)

Inductive bstep': prog -> rel state state :=
| s_skip: forall s, bstep' skip s s
| s_ass: forall x e s, bstep' (x <- e) s (update x (e s) s)
| s_seq: forall p q s s' s'', bstep' p s s' -> bstep' q s' s'' -> bstep' (p ;; q) s s''
| s_if_ins_ff: forall (b: dset state) p q s s', b s = false -> bstep' q s s' -> bstep' (if_ins b p q) s s'
| s_if_ins_tt: forall (b: dset state) p q s s', b s = true -> bstep' p s s' -> bstep' (if_ins b p q) s s'
| s_while_ins_ff: forall (b: dset state) p s, b s = false -> bstep' (while_ins b p) s s
| s_while_ins_tt: forall (b: dset state) p s s', b s = true -> bstep' (p ;; while_ins b p) s s' -> bstep' (while_ins b p) s s'.
...
Proof.
  ...
  - induction p; unfold bstep; fold bstep.
     ...
     intros ? ? [[? [<- H] H']|[? [<- H] H']]. 
      apply s_if_ins_tt. assumption. apply IHp1, H'. 
      apply s_if_ins_ff. now apply Bool.negb_true_iff. apply IHp2, H'. 
     apply str_ind_l'.
      intros ? ? [<- H]. apply s_while_ins_ff. now apply Bool.negb_true_iff.
      rewrite <-dotA. intros s s'' [? [<- H] [s' H' H'']]. apply s_while_ins_tt. assumption.
...
Lemma two_loops b p: 
  while_ins b (while_ins b p)  ~  while_ins b p.
Proof. simp. kat. Qed.

(** ** folding a loop *)
Lemma fold_loop b p: 
  while_ins b (p ;; if_ins b p skip)  ~  
  while_ins b p.
Proof. simp. kat. Qed.

(** ** eliminating deadcode *)
Lemma dead_code b p q r: 
  (while_ins b p ;; if_ins b q r)  ~  
  (while_ins b p ;; r).
Proof. simp. kat. Qed.

Lemma dead_code' a b p q r: 
  (while_ins (a \cup b) p ;; if_ins b q r)  ~ 
  (while_ins (a \cup b) p ;; r).
Proof. simp. kat. Qed.
...
Lemma aff_ite x e t p q: 
  (x <- e ;; if_ins t p q)  
  ~ 
  (if_ins (subst x e t) (x <- e ;; p) (x <- e ;; q)).
Proof.
  simp. 
...
Lemma rule_skp A: Hoare A skip A.
Proof. simp. kat. Qed.
...
Lemma rule_ite A B t p q: 
  Hoare (A \cap t) p B -> 
  Hoare (A \cap !t) q B -> 
  Hoare A (if_ins t p q) B.
Proof. simp. hkat. Qed.

Lemma rule_whl A t p: 
  Hoare (A \cap t) p A -> 
  Hoare A (while_ins t p) (A \cap neg t).
Proof. simp. hkat. Qed.
